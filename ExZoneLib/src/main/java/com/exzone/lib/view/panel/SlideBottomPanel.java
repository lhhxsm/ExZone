...
public class SlideBottomPanel extends FrameLayout {
  ...
  public SlideBottomPanel(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.SlideBottomPanel, defStyleAttr, 0);

    mBackgroundId =
        a.getResourceId(R.styleable.SlideBottomPanel_sbp_background_layout, DEFAULT_BACKGROUND_ID);
    mPanelHeight = a.getDimension(R.styleable.SlideBottomPanel_sbp_panel_height,
        ScreenUtils.dp2px(mContext, DEFAULT_PANEL_HEIGHT));
    mBoundary = a.getBoolean(R.styleable.SlideBottomPanel_sbp_boundary, DEFAULT_BOUNDARY);
    MAX_CLICK_DISTANCE = mTitleHeightNoDisplay =
        a.getDimension(R.styleable.SlideBottomPanel_sbp_title_height_no_display,
            ScreenUtils.dp2px(mContext, DEFAULT_TITLE_HEIGHT_NO_DISPLAY));
    mMoveDistanceToTrigger = a.getDimension(R.styleable.SlideBottomPanel_sbp_move_distance_trigger,
        ScreenUtils.dp2px(mContext, DEFAULT_MOVE_DISTANCE_TO_TRIGGER));
    mAnimationDuration =
        a.getInt(R.styleable.SlideBottomPanel_sbp_animation_duration, DEFAULT_ANIMATION_DURATION);
    mHidePanelTitle =
        a.getBoolean(R.styleable.SlideBottomPanel_sbp_hide_panel_title, DEFAULT_HIDE_PANEL_TITLE);
  ...
  private void handleActionUp(MotionEvent event) {
    ...
    computeVelocity();
    if (!isPanelShowing && ((event.getY() - firstDownY) < 0 && (Math.abs(event.getY() - firstDownY)
        > mMoveDistanceToTrigger)) || (yVelocity < 0
    ...
    if (isPanelShowing) {
      ...
      if (currentY < (mMeasureHeight - mPanelHeight) || currentY < (mMeasureHeight - mPanelHeight
  ...
  private void handleActionMove(MotionEvent event) {
    ...
    if (!isDragging
        && Math.abs(event.getY() - firstDownY) > mTouchSlop
    ...
    if (isDragging) {
      ...
      if (mDarkFrameLayout != null && mIsFade) {
        ...
        if (currentY > mMeasureHeight - mPanelHeight
          ...
          mDarkFrameLayout.fade((int) ((1 - currentY / (mMeasureHeight - mTitleHeightNoDisplay))
      ...
      if (!mBoundary) {
        ...
        if (touchingViewY + deltaY <= mMeasureHeight - mPanelHeight) {
          ...
          touchingView.offsetTopAndBottom(
              (int) (mMeasureHeight - mTitleHeightNoDisplay - touchingViewY));
        } else {
          touchingView.offsetTopAndBottom((int) deltaY);
        }
      }
  ...
  private void hidePanel() {
    ...
    ValueAnimator animator =
        ValueAnimator.ofFloat(ViewHelper.getY(mPanel), mMeasureHeight - mTitleHeightNoDisplay);
  ...
  private void displayPanel() {
    ...
    final View mPanel = findViewWithTag(TAG_PANEL);
    ValueAnimator animator =
        ValueAnimator.ofFloat(ViewHelper.getY(mPanel), mMeasureHeight - mPanelHeight)
            .setDuration(mAnimationDuration);
    animator.setTarget(mPanel);
    ...
    animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
      @Override public void onAnimationUpdate(ValueAnimator animation) {
        ...
        if (mDarkFrameLayout != null
          ...
          mDarkFrameLayout.fade((int) ((1 - value / (mMeasureHeight - mTitleHeightNoDisplay))
              * DarkFrameLayout.MAX_ALPHA));
        }
      }
    });
