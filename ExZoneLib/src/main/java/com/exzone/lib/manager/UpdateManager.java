...
public class UpdateManager {
    ...
    public static void checkUpdate(final BaseActivity context, final boolean isForceCheck) {
        if (!NetWorkUtils.isConnected(context)) {
            ...
            if (isForceCheck) {
                // 手动强制检测更新时，提示文字
                ToastUtils.showShort(context, context.getString(R.string.please_check_the_network_connection));
            } else {
                // 非强制不做操作
            }
            return;
     ...
     * 无Wifi状态确认更新对话框
     */
    private static void showNoWifiConfirmDialog(final BaseActivity context, final UpdateEntity updateEntity) {
     ...
     * 显示更新对话框,包含版本相关信息
     */
    private static void showUpdateConfirmDialog(final BaseActivity context, final UpdateEntity updateEntity) {
