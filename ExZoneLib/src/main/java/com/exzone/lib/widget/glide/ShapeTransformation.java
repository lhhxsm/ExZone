...
public class ShapeTransformation extends BitmapTransformation {
    // 使用方法
    // Glide.with(this).load(url).transform(new ShapeTransformation(this, R.drawable.xxx)).into(imageView);
    // 或
    // Glide.with(this).load(url).bitmapTransform(new ShapeTransformation(this, R.drawable.xxx)).into(imageView);
    private static Paint mPaint = new Paint(); // 画笔
    private Context mContext;
    private int mShapeRes; // 形状的drawable资源

    public ShapeTransformation(Context context, int shapeRes) {
        super(context);
        mContext = context;
        mShapeRes = shapeRes;
    ...
    public String getId() {
        // 用于缓存的唯一标识符
        return "ShapeTransformation" + mShapeRes;
    }
}
