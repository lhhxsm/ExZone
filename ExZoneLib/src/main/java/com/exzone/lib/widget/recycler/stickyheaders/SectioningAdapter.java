...
public class SectioningAdapter extends RecyclerView.Adapter<SectioningAdapter.ViewHolder> {
  ...
  /**
   * Called to update the ghost header for a particular section. Note, most implementations will not
   * need to ever touch the ghost header.
   *
   * @param viewHolder the view holder to update
  ...
  public int getSectionForAdapterPosition(int adapterPosition) {
    ...
    if (adapterPosition < 0 || adapterPosition >= getItemCount()) {
      throw new IndexOutOfBoundsException("adapterPosition "
          + adapterPosition
          + " is not in range of items represented by adapter");
    }
  ...
  /**
   * Given a sectionIndex and an adapter position get the local position of an item relative to the
   * sectionIndex,
  ...
  /**
   * Given a sectionIndex index, and an offset into the sectionIndex where 0 is the header, 1 is the
   * ghostHeader, 2 is the first item in the sectionIndex, return the corresponding "global" adapter
   * position
   *
   * @param sectionIndex a sectionIndex index
   * @param offsetIntoSection offset into sectionIndex where 0 is the header, 1 is the first item,
   * etc
   ...
   *
   * @param sectionIndex the index of the section
   * @param offsetIntoSection the offset of the item in the section where 0 would be the first item
   * in the section
   ...
   * to the item's section, and R.id.sectioning_adapter_tag_key_view_adapter_position which is the
