package com.exzone.lib.widget.recycler.decoration.grid;
...
import com.exzone.lib.widget.recycler.recyclerview.HeaderAndFooterRecyclerViewAdapter;
  ...
  @Override public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (child != null) {
        ...
        final int top =
            child.getTop() + params.topMargin + Math.round(ViewCompat.getTranslationY(child));
        final int bottom =
        ...
        if (childPosition >= builder.spanCount) {
          ...
          final int left1 =
              child.getLeft() + params.leftMargin + Math.round(ViewCompat.getTranslationX(child));
          final int right1 = child.getRight() + params.rightMargin + Math.round(
              ViewCompat.getTranslationX(child) + builder.h_spacing);
          final int top1 =
              child.getTop() + params.topMargin + Math.round(ViewCompat.getTranslationY(child))
                  - builder.v_spacing;
